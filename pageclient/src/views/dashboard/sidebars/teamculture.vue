<template>
    <v-data-table
      ...
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Lab TeamCulture</v-toolbar-title>
                  ...
                  <v-row>
                    <v-col cols="9">
                      ...
                      <v-list-item-subtitle>
                        <v-text-field auto-grow :row-height="3" label="图片链接" v-model="editedItem.image"></v-text-field>
                      </v-list-item-subtitle>
                    </v-col>
                    <v-col cols="3">
                      <v-btn
                    ...
                    <v-col cols="12">
                      <v-text-field label="reserarch title" v-model="editedItem.title">
                      </v-text-field>
                    </v-col>
                    <v-col cols="6">
      ...
      <template v-slot:item.title="{ item }">
        <Cell :content="item.title" :width="'150px'" :lines="2"/>
      </template>
  ...
  import { createTeamCultureAPI, deleteTeamCultureAPI, getTeamCultureListAPI, updateTeamCultureAPI } from '@/utils/api/teamculture'
  ...
  export default {
    ...
    data: () => ({
      ...
      headers: [
        ...
        { text: '外链', value: 'outsideUrl',sortable: false },
        { text: '内链', value: 'insideUrl',sortable: false },
    ...
    methods: {
      ...
      async flashTable(){
        ...
        try{
          const res = await getTeamCultureListAPI({currentPage:this.footBarParams.currentPage,size:this.footBarParams.size})
          if(res.code==200){
      ...
      async addItem(data){
        await createTeamCultureAPI(data)
      },
  
      async updateItem(data){
        await updateTeamCultureAPI(data)
      },
      ...
      async deleteItemConfirm() {
        await deleteTeamCultureAPI(this.editedItem.id)
        this.closeDelete()
      },
      ...
      async cropperConfirmChange(e){
        if(e){
            ...
            if(res&&res.code&&res.code===200){
              this.editedItem.image=res.data
            }
        }
        this.dialogCropper=false
      }
